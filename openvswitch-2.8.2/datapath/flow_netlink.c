				...
				bool is_mask, bool log)
{
	...
	if (attrs & (1ULL << OVS_KEY_ATTR_TCP)) {
		...
		SW_FLOW_KEY_PUT(match, tp.flags, tcp_key->tcp_flags, is_mask);
        SW_FLOW_KEY_PUT(match, tp.seq, tcp_key->tcp_seq, is_mask);
        SW_FLOW_KEY_PUT(match, tp.ack, tcp_key->tcp_ack, is_mask);
			     ...
			     struct sk_buff *skb)
{
	...
	if ((swkey->eth.type == htons(ETH_P_IP) ||
		...
		if (swkey->ip.proto == IPPROTO_TCP) {
            ...
            tcp_key->tcp_flags = output->tp.flags;
            tcp_key->tcp_seq = output->tp.seq;
            tcp_key->tcp_ack = output->tp.ack;
