                        ...
                        struct ofpbuf *key, struct ofpbuf *mask)
{
    ...
    SCAN_BEGIN("tcp(", struct ovs_key_tcp) {
        ...
        SCAN_FIELD("seq=", be32, tcp_seq);
        SCAN_FIELD("ack=", be32, tcp_ack);
...
get_tp_key(const struct flow *flow, union ovs_key_tp *tp)
{
    ...
    tp->tcp.tcp_flags = flow->tcp_flags;
...
put_tp_key(const union ovs_key_tp *tp, struct flow *flow)
{
    ...
    flow->tcp_flags = tp->tcp.tcp_flags;
    flow->tcp_seq = tp->tcp.tcp_seq;
    flow->tcp_ack = tp->tcp.tcp_ack;
}
