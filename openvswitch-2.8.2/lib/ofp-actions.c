...
/* ## ----------------- ## */
    ...
    /* OF1.0(12) is deprecated (use Set-Field):
     * ovs_be16. */
    OFPAT_RAW_SET_TCP_FLAGS,

    /* OF1.0(13) is deprecated (use Set-Field):
     * ovs_be32. */
    OFPAT_RAW_SET_TCP_SEQ,

    /* OF1.0(14) is deprecated (use Set-Field):
     * ovs_be32. */
    OFPAT_RAW_SET_TCP_ACK,
...
ofpact_next_flattened(const struct ofpact *ofpact)
{
    ...
    case OFPACT_SET_L4_TCP_FLAGS:
    case OFPACT_SET_L4_TCP_SEQ:
    case OFPACT_SET_L4_TCP_ACK:
...

/* Set TCP flags/seq/ack actions */

static enum ofperr
decode_OFPAT_RAW_SET_TCP_FLAGS(ovs_be16 tcp_flags,
                               enum ofp_version ofp_version OVS_UNUSED,
                               struct ofpbuf *out)
{
    ofpact_put_SET_L4_TCP_FLAGS(out)->tcp_flags = ntohs(tcp_flags);
    return 0;
}

static enum ofperr
decode_OFPAT_RAW_SET_TCP_SEQ(ovs_be32 tcp_seq,
                             enum ofp_version ofp_version OVS_UNUSED,
                             struct ofpbuf *out)
{
    ofpact_put_SET_L4_TCP_SEQ(out)->tcp_seq = ntohl(tcp_seq);
    return 0;
}

static enum ofperr
decode_OFPAT_RAW_SET_TCP_ACK(ovs_be32 tcp_ack,
                             enum ofp_version ofp_version OVS_UNUSED,
                             struct ofpbuf *out)
{
    ofpact_put_SET_L4_TCP_ACK(out)->tcp_ack = ntohl(tcp_ack);
    return 0;
}

static void
encode_SET_L4_flags(const struct ofpact_l4_flags *l4_flags,
                    enum ofp_version ofp_version, enum ofp_raw_action_type raw,
                    enum mf_field_id field, struct ofpbuf *out)
{
    ovs_be16 flags = l4_flags->tcp_flags;

    if (ofp_version >= OFP12_VERSION) {
        put_set_field(out, ofp_version, field, flags);
    } else {
        ofpact_put_raw(out, ofp_version, raw, flags);
    }
}

static void
encode_SET_L4_seq(const struct ofpact_l4_seq *l4_seq,
                  enum ofp_version ofp_version, enum ofp_raw_action_type raw,
                  enum mf_field_id field, struct ofpbuf *out)
{
    ovs_be32 seq = l4_seq->tcp_seq;
    
    if (ofp_version >= OFP12_VERSION) {
        put_set_field(out, ofp_version, field, seq);
    } else {
        ofpact_put_raw(out, ofp_version, raw, seq);
    }
}

static void
encode_SET_L4_ack(const struct ofpact_l4_ack *l4_ack,
                  enum ofp_version ofp_version, enum ofp_raw_action_type raw,
                  enum mf_field_id field, struct ofpbuf *out)
{
    ovs_be32 ack = l4_ack->tcp_ack;

    if (ofp_version >= OFP12_VERSION) {
        put_set_field(out, ofp_version, field, ack);
    } else {
        ofpact_put_raw(out, ofp_version, raw, ack);
    }
}

static void
encode_SET_L4_TCP_FLAGS(const struct ofpact_l4_flags *l4_flags,
                        enum ofp_version ofp_version,
                        struct ofpbuf *out)
{
    enum mf_field_id field = MFF_TCP_FLAGS;
    encode_SET_L4_flags(l4_flags, ofp_version, OFPAT_RAW_SET_TCP_FLAGS, field, out);
}

static void
encode_SET_L4_TCP_SEQ(const struct ofpact_l4_seq *l4_seq,
                      enum ofp_version ofp_version,
                      struct ofpbuf *out)
{
    enum mf_field_id field = MFF_TCP_SEQ;
    encode_SET_L4_seq(l4_seq, ofp_version, OFPAT_RAW_SET_TCP_SEQ, field, out);
}

static void
encode_SET_L4_TCP_ACK(const struct ofpact_l4_ack *l4_ack,
                      enum ofp_version ofp_version,
                      struct ofpbuf *out)
{
    enum mf_field_id field = MFF_TCP_ACK;
    encode_SET_L4_ack(l4_ack, ofp_version, OFPAT_RAW_SET_TCP_ACK, field, out);
}

static char * OVS_WARN_UNUSED_RESULT
parse_SET_L4_TCP_FLAGS(char *arg,
                       const struct ofputil_port_map *port_map OVS_UNUSED,
                       struct ofpbuf *ofpacts,
                       enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    return str_to_u16(arg, "tcp flags",
                      &ofpact_put_SET_L4_TCP_FLAGS(ofpacts)->tcp_flags);
}

static char * OVS_WARN_UNUSED_RESULT
parse_SET_L4_TCP_SEQ(char *arg,
                     const struct ofputil_port_map *port_map OVS_UNUSED,
                     struct ofpbuf *ofpacts,
                     enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    return str_to_u32(arg,
                      &ofpact_put_SET_L4_TCP_SEQ(ofpacts)->tcp_seq);
}

static char * OVS_WARN_UNUSED_RESULT
parse_SET_L4_TCP_ACK(char *arg,
                     const struct ofputil_port_map *port_map OVS_UNUSED,
                     struct ofpbuf *ofpacts,
                     enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    return str_to_u32(arg,
                      &ofpact_put_SET_L4_TCP_ACK(ofpacts)->tcp_ack);
}

static void
format_SET_L4_TCP_FLAGS(const struct ofpact_l4_flags *a,
                        const struct ofputil_port_map *port_map OVS_UNUSED,
                        struct ds *s)
{
    ds_put_format(s, "%smod_tcp_flags:%s%d", colors.param, colors.end, a->tcp_flags);
}

static void
format_SET_L4_TCP_SEQ(const struct ofpact_l4_seq *a,
                      const struct ofputil_port_map *port_map OVS_UNUSED,
                      struct ds *s)
{
    ds_put_format(s, "%smod_tcp_seq:%s%d", colors.param, colors.end, a->tcp_seq);
}

static void
format_SET_L4_TCP_ACK(const struct ofpact_l4_ack *a,
                      const struct ofputil_port_map *port_map OVS_UNUSED,
                      struct ds *s)
{
    ds_put_format(s, "%smod_tcp_ack:%s%d", colors.param, colors.end, a->tcp_ack);
}

                             ...
                             struct ofpbuf *out)
{
    ...
    /*case MFF_TCP_FLAGS:
        put_OFPAT_SET_TCP_FLAGS(out, sf->value->be16);
        break;

    case MFF_TCP_SEQ:
        put_OFPAT_SET_TCP_SEQ(out, sf->value->be32);
        break;

    case MFF_TCP_ACK:
        put_OFPAT_SET_TCP_ACK(out, sf->value->be32);
        break;*/
...
get_ofpact_map(enum ofp_version version)
{
    ...
    static const struct ofpact_map of10[] = {
        ...
        { OFPACT_ENQUEUE, 11 },
        { OFPACT_SET_L4_TCP_FLAGS, 12 },
        { OFPACT_SET_L4_TCP_SEQ, 13 },
        { OFPACT_SET_L4_TCP_ACK, 14 },
        { 0, -1 },
    };
