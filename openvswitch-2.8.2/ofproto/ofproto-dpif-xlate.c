    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_SET_L4_TCP_FLAGS:
            if(is_ip_any(flow)) {
                ...
                flow->tcp_flags = htons(ofpact_get_SET_L4_TCP_FLAGS(a)->tcp_flags);
            }
            break;

        case OFPACT_SET_L4_TCP_SEQ:
            if(is_ip_any(flow)) {
                memset(&wc->masks.tcp_seq, 0xff, sizeof wc->masks.tcp_seq);
                flow->tcp_seq = htonl(ofpact_get_SET_L4_TCP_SEQ(a)->tcp_seq);
            }
            break;

        case OFPACT_SET_L4_TCP_ACK:
            if(is_ip_any(flow)) {
                memset(&wc->masks.tcp_ack, 0xff, sizeof wc->masks.tcp_ack);
                flow->tcp_ack = htonl(ofpact_get_SET_L4_TCP_ACK(a)->tcp_ack);
            }
            break;
